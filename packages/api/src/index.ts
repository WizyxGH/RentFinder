/**
 * API RentFinder — Cloudflare Worker (§28).
 *
...
import { corsHeaders, jsonError, requireAuth } from './auth.js';
...
async function recordContact(
  ...
  await db.execute({
    sql: 'UPDATE listings SET tracking = ?, updated_at = ? WHERE id = ?',
    args: [followUpIndex === 0 ? 'contacted' : 'contacted', now, id],
  });
 ...
 *
 * Séparé du gestionnaire principal, qui ne s'occupe plus que du protocole
 * (CORS, authentification, gestion d'erreur). Chacun reste ainsi lisible et
 * testable isolément (§75).
 *
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (segments[0] !== 'api') return json({ error: 'Route inconnue' }, cors, 404);
    ...
    try {
      ...
      return json({ error: 'Erreur interne' }, cors, 500);
    }
  },
};
